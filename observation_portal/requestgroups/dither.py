...
def expand_dither_pattern(configuration_dict, dither_details):
    ...
    if pattern == 'line':
        ...
        offsets = calc_box_offsets(dither_details.get('num_points'), dither_details.get('point_spacing'), dither_details.get('point_spacing'),
        ...
        offsets = calc_grid_offsets(dither_details.get('num_rows'), dither_details.get('num_columns'), dither_details.get('point_spacing'),
                                    dither_details.get('orientation'), dither_details.get('center'))
    ...
    for offset in offsets:
        ...
        instrument_config_copy['extra_params']['offset_ra'] = offset[0]
        instrument_config_copy['extra_params']['offset_dec'] = offset[1]
...
def calc_box_offsets(num_points, point_spacing, line_spacing, orient=0, sides_angle=-90):
...
def calc_grid_offsets(num_rows, num_columns, point_spacing, orient=90, center=False):
    ...
    row_distance_offset = -(point_spacing * (num_columns-1)) / 2.0 if center else 0.0
    ...
    row_x_distance = point_spacing * sin(radians(360-(90-orient)))
    row_y_distance = point_spacing * cos(radians(360-(90-orient)))
