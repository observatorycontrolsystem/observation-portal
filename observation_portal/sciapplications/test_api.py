...
class TestCallAPI(APITestCase):
    def setUp(self) -> None:
        now = timezone.now()
        self.open_call = mixer.blend(Call, opens=now - timedelta(days=1), deadline=now + timedelta(days=1))
        self.closed_call = mixer.blend(Call, opens=now - timedelta(days=2), deadline=now - timedelta(days=1))
